...
try:
    ...
    cur.execute("use AgentIp")
    cur.execute(
        "create table ipList (ip varchar(20), port varchar(6), area varchar(20), primary key(ip)) character set = utf8")
    conn.commit()
    ...
    try:
        if(opener.open("http://5sing.kugou.com/index.html").code == 200):
            try:
                #print("验证成功" + str(d))
                mu.acquire()
                cur.execute("insert into ipList (ip, port,area) values (\"%s\", \"%s\", \"%s\")", (ip, str(
                    port), str(area)))
                conn.commit()
                print("插入成功" + str(d))
                mu.release()
...
def main():
    for area in range(1, 35):
        for page in range(1, 5):
            str_url = "http://www.66ip.cn/areaindex_" + \
                str(area) + "/" + str(page) + ".html"
