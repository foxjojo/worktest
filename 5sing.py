...
def store(title, renqi, songid, httpstate):
    try:
        ...
        # print(type(title))
        # print(type(renqi))
        cur.execute("insert into song (title, renqi, songid, httpstate) values (\"%s\", \"%s\", \"%s\", \"%s\")",
                    (title, renqi, songid, httpstate))
        conn.commit()
        lock.release()
    except Exception as e:
        lock.release()
...
def handle_html(songid, opener):
    try:
        ...
        try:
            js = opener.open(
                "http://service.5sing.kugou.com/analysis/songGraph?jsoncallback=type=1&SongID=" + str(songid) + "&SongType=yc")
            strjs = js.read()
            strjs = strjs[7:]
            strjs = strjs[:-1]
            b = eval(strjs)
